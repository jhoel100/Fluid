...
Fluid::Fluid(int n, float r) {
	...
	kernel_radius = r;
	rest_density = 0.05;
	fluid_constant = 15000;

	int sep = 60;

	for (int i = 0; i < num_particles; i++) {
		//Posicion del bloque de agua
		Particle initializer = Particle((i / sep) * 1.8, (i % sep) * 1.8 + 80, 0.5, 0, rest_density, fluid_constant);
	...
	//volumnes finitos
	optimize = new SpatialGrid(500, 500, kernel_radius);
...
void Fluid::color() {
	for (int i = 0; i < num_particles; i++) {
		Particles[i].col = sf::Color(0,60, 255, 150);
	}
}
...
void Fluid::walls(float dt) {
	for (int i = 0; i < num_particles; i++) {
		...
		if (Particles[i].getX() >= 229) {
			Particles[i].setX(228.9);
			Particles[i].setU(Particles[i].getU() * -0.5);
		}
